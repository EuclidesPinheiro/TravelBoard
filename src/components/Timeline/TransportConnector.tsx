...
export function TransportConnector({ segment, traveler, left, width }: TransportConnectorProps) {
  ...
  return (
    ...
    >
      {/* Dashed lines */}
      <div className="absolute top-1/2 bottom-0 left-0 border-l border-dashed border-red-400 pointer-events-none" />
      <div className="absolute top-1/2 bottom-0 right-0 border-r border-dashed border-red-400 pointer-events-none" />
      
      {/* The connector block */}
      <div
        className={cn(
          "absolute top-1/2 -translate-y-1/2 left-0 right-0 h-4 rounded-full border-2 border-slate-800 flex items-center justify-center cursor-pointer transition-all hover:shadow-md hover:scale-110",
